...
PURPOSE: This script will tell you whether crawl, index and serving
...
date. If this is a big difference, then something is wrong, otherwise
...
installed. To run the script, run: python
cached_copy_checker.py). Feel free to change send_alert message to
just print the error instead of emailing it during the first test run.
'''
...
subject = 'Email from Google Appliance (%s) - Cache copy checker' %appliance_hostname
...
try:
  cached_url="http://"+appliance_hostname+"/search?q=cache:"+urllib.quote_plus(url)
  ...
  try:
    ...
    raise ValueError("Did not find time in the cached copy. It is possible that document has not yet been indexed. Please try accessing the URL directly. Here is the content I got :\n%s " %content)
  ...
  if lag > ACCEPTABLE_LAG:
    message = 'Cached copy is older than %s days! - %s days old' %(ACCEPTABLE_LAG/ONE_DAY,float(lag)/float(ONE_DAY))
    print message
    print 'sending an alert'
    send_alert(cached_url,message)
  else:
    print 'Cached copy is less than %s days old. Everything is good!' %(ACCEPTABLE_LAG/ONE_DAY)
except Exception, exception:
  print 'Got exception. Sending an alert'
